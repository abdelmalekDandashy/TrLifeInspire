...
export class TripComponent implements OnInit, OnDestroy {
    Get_Trip_By_Where_Subscription = new Subscription();
    searchModel: Params_Get_Trip_By_Where = new Params_Get_Trip_By_Where();
    data: Trip[] = [];
    ...
    ngOnDestroy(): void {
        this.Get_Trip_By_Where_Subscription.unsubscribe();
    ...
    fetchData() {
        ...
        this.Get_Trip_By_Where_Subscription = this.proxy.Get_Trip_By_Where(this.searchModel).subscribe(result => {
            if (result.My_Result != null) {
                result.My_Result.forEach((element: any) => {
                    element.MyURL = this.CmSvc.APIUrl + '/Upload_Image?REL_ENTITY=[TBL_TRIP]&REL_FIELD=TRIP_IMAGE&REL_KEY=' + element.TRIP_ID;
    ...
    AddEntry() {
        if (this.data !== undefined) {
            if (this.data.filter(e => e.TRIP_ID === -1).length > 0) {
                return;
            }
        }
        const record = new Trip();
        record.TRIP_ID = -1;
        record.IS_AVAILABLE_OR_COMING_SOON = false;
        this.data.unshift(record);
    }
    Edit(current) {
        this.proxy.Edit_Trip(current).subscribe((result) => {
            if (result != null) {
                this.CmSvc.ShowMessage('Done');
                if (current.TRIP_ID === -1) {
                    this.data.splice(this.data.indexOf(current), 1);
                    const newEntry: any = result;
                    newEntry.MyUploadedImages = [];
                    newEntry.MyURL = this.CmSvc.APIUrl + '/Upload_Image?REL_ENTITY=[TBL_TRIP]&REL_FIELD=TRIP_IMAGE&REL_KEY=' + newEntry.TRIP_ID;
    ...
    Delete(entry) {
        ...
        dialogRef.afterClosed().subscribe(response => {
            if (response) {
                const _params_Delete_Trip = new Params_Delete_Trip();
                _params_Delete_Trip.TRIP_ID = entry.TRIP_ID;
                this.proxy.Delete_Trip(_params_Delete_Trip).subscribe(data => {
                    if (data === '') {
